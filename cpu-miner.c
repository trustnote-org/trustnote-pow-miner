...
int g_timeout;
char ws_buff[2048];
static pthread_mutex_t g_ws_lock;
...
static bool gbt_work_decode(const json_t *val, struct work *work)
{
	int n = 0, number;
	json_t *tmp;
	uint32_t data[41];	// 164 bytes / 4 = 41
	uint32_t target[8];
	uint8_t pubSeed[32];		// 作为返回值
	char errMsg[64];
	char *pPubSeed = NULL;
	char *pPubkey = NULL;
	char *pDiff = NULL;
	...
	if (!tmp || !json_is_integer(tmp)) {
		snprintf(errMsg, sizeof(errMsg), "JSON invalid version");
		applog(LOG_ERR, errMsg);
		goto OUT;
	}
	work->version = (int) json_integer_value(tmp);
	data[0] = (uint32_t)work->version;
	if(opt_debug)
		applog(LOG_BLUE, "Current version is %d", work->version);

	// nonce
	tmp = json_object_get(val, "nonce");
	if (!tmp || !json_is_integer(tmp)) {
		snprintf(errMsg, sizeof(errMsg), "JSON invalid nonce");
		applog(LOG_ERR, errMsg);
		goto OUT;
	}
	work->nonce = (uint32_t) json_integer_value(tmp);
	if(opt_debug)
		applog(LOG_BLUE, "Current nonce is %d", work->nonce);

	// times
	tmp = json_object_get(val, "times");
	if (!tmp || !json_is_integer(tmp)) {
		snprintf(errMsg, sizeof(errMsg), "JSON invalid times");
		applog(LOG_ERR, errMsg);
		goto OUT;
	}
	work->times = (uint32_t)json_integer_value(tmp);

	// timeout
	tmp = json_object_get(val, "timeout");
	if (!tmp || !json_is_integer(tmp)) {
		snprintf(errMsg, sizeof(errMsg), "JSON invalid timeout");
		applog(LOG_ERR, errMsg);
		goto OUT;
	}
	work->timeout = (uint32_t)json_integer_value(tmp);

	// roundNumber
	tmp = json_object_get(val, "roundNumber");
	if (!tmp || !json_is_integer(tmp)) {
		snprintf(errMsg, sizeof(errMsg), "JSON invalid roundNumber");
		applog(LOG_ERR, errMsg);
		goto OUT;
	}
	number = data[1] = (uint32_t)json_integer_value(tmp);			// 4 bytes

	if(opt_debug)
		applog(LOG_BLUE, "Current roundNumber is %d\n", number);

	if(work->timeout > 0 && timestamp <= work->timeout)
		{
		restart_threads();
	} else if (work->times > 0 && g_nonce >= work->nonce + work->times)
		{
		restart_threads();
	} else if (round_number < number)		// 轮次号更新后，停止所有挖矿线程任务，重新分配新任务
		{
		restart_threads();
		round_number = number;
		g_nonce = work->nonce;
		...
		{
		g_timeout = work->timeout;
		pthread_mutex_lock(&g_ws_lock);
		work->nonce = g_nonce = g_nonce + 10;
		pthread_mutex_unlock(&g_ws_lock);
	}

	// prepubSeed	
	tmp = json_object_get(val, "pubSeed");
	if (!tmp || !json_is_string(tmp)) {
		snprintf(errMsg, sizeof(errMsg), "JSON invalid prepubSeed");
		applog(LOG_ERR, errMsg);
		goto OUT;
	}
	pPubSeed = (char *)json_string_value(tmp); // 00000000000000000015a0f5afb0006b9415bc781fa5ce78115a6d07d5a10010
	if(!strncasecmp(pPubSeed, "0x", 2))
		pPubSeed += 2;
	if(opt_debug)
		applog(LOG_BLUE, "Current prepubSeed is %s\n", pPubSeed);

	// 录入 prepubSeed
	for(int i = 0; i < 16; i++){
		sscanf(pPubSeed + i*8, "%8x", &data[2+i]);		// 32 bytes
	}
	
	// pubkey
	tmp = json_object_get(val, "pubKey");
	if (!tmp || !json_is_string(tmp)) {
		snprintf(errMsg, sizeof(errMsg), "JSON invalid pubkey");
		applog(LOG_ERR, errMsg);
		goto OUT;
	}
	pPubkey = (char *)json_string_value(tmp);
	if(!strncasecmp(pPubkey, "0x", 2))
		pPubkey += 2;

	// difficulty
	tmp = json_object_get(val, "difficulty");
	if (!tmp || !json_is_integer(tmp)) {
		snprintf(errMsg, sizeof(errMsg), "JSON invalid times");
		applog(LOG_ERR, errMsg);
		goto OUT;
	}
	work->difficulty = (uint32_t)json_integer_value(tmp);
	
	
	if(opt_algo == ALGO_EQUIHASH){

		//blake2b(work->pubSeed, (const void *)data, NULL, 32, sizeof(data), 0);
		blake2b((uint8_t*)work->data, (const void *)data, pPubkey, 140, 72, strlen(pPubkey));	// 32 + 4 + 4
	}
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	hashhex = abin2hex(work->hash, sizeof(work->hash));

	snprintf(s, JSON_BUF_LEN,
			"{\"id\": %d,\"method\": \"%s\",\"pow\": \"%s\",\"params\": {\"version\": %d,\"roundNumber\": %d,\"curtime\": %d,\"nonce\": %d,\"result\": \"%s\"},\"error\": \"%s\"}",
			work->id, "submit", "equihash", 0, work->roundNumber, work->curtime, work->nonce, hashhex);
	free(hashhex);
...
start:
	...
	if (opt_algo == ALGO_EQUIHASH)
		rc = gbt_work_decode(json_object_get(val, "params"), work);
...
static void *workio_thread(void *userdata)
{
	...
	while (ok) {
		...
		if (!wc) {
			applog(LOG_ERR, "get cmd failed, break.");
			ok = false;
			break;
		}
		...
		applog(LOG_BLUE, "after get pop work, ok = %d\n", ok);
...
static void *miner_thread(void *userdata)
{
	...
	struct work work;
	uint32_t max_nonce;
	sscanf("0xffffffff", "%8x", &max_nonce);
	...
	if (opt_algo == ALGO_SCRYPT) {
		...
		scratchbuf = (uchar*)malloc(opt_pluck_n * 1024);
		if (!scratchbuf) {
	...
	applog(LOG_BLUE, "thread %d start work.\n\n", thr_id);
	...
	while (1) {
		...
		if (unlikely(!get_work(mythr, &work))) {
			applog(LOG_ERR, "work retrieval failed, retrying "
		...
		max_nonce = 10 + work.nonce;

		//if(opt_debug)
		applog(LOG_BLUE, "thread %d start work, max_nonce = %d, nonce = %d.", thr_id, max_nonce, work.nonce);
		...
		if (!opt_quiet && (time(NULL) - tm_rate_log) > opt_maxlograte) {
			switch(opt_algo) {
			case ALGO_EQUIHASH:
				applog(LOG_INFO, "hashes_done = %d, tv_sec = %d, tv_usec = %d",
						hashes_done,
						diff.tv_sec,
						diff.tv_usec);
				break;
	...
	case 'a':
		...
		if (i == ALGO_COUNT) {
			...
			if (!strcasecmp("blake2", arg))
				...
				i = opt_algo = ALGO_ZR5;
			else if (!strcasecmp("equihash", arg))
				i = opt_algo = ALGO_EQUIHASH;
			else
...
static int thread_create(struct thr_info *thr, void* func)
{
	...
	err = pthread_create(&thr->pth, &thr->attr, (FUNC)func, thr);
...
int main(int argc, char *argv[]) {
	...
	if (!opt_n_threads)
		opt_n_threads = num_cpus; // num_cpus
	...
	pthread_mutex_init(&g_ws_lock, NULL);
	...
	if (thread_create(thr, (void*)workio_thread)) {
	...
	if (want_longpoll && !have_stratum) {
		...
		err = thread_create(thr, (void*)longpoll_thread);
		if (err) {
	...
	for (i = 0; i < opt_n_threads; i++) {
		...
		err = thread_create(thr, (void*)miner_thread);
		if (err) {
