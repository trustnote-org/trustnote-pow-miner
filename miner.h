...
extern json_t *ws_json_decode(pthread_mutex_t *ws_lock, char *payload);
...
bool diffcheck(const uint8_t *hash, const uint32_t *target);
...
struct work {
	int 		id;					// rpc id
	int			version;			// 挖矿版本
	uint32_t	nonce;				// 初始nonce
	uint32_t	difficulty;			// 难度值
	char*		roundNumber;		// 轮次标识
	char* 		prepubSeed;			// 上一轮次的公共种子
	char*		prepreCoinbase;		// 上上个轮次的coinbase
	char*		preFirstStableMCU;	// 上一个轮次的第一个稳定主链公正单元
	char*		pubkey;				// 发起挖矿请求的超级节点的公钥
	int			times;				// 最大运算次数. 0, 表示不限制运算次数
	int			timeout;			// 挖矿超时时间，单位：秒

	uint32_t 	_ALIGN(64) data[48];
	uint32_t 	_ALIGN(64) target[8];
	uint8_t		_ALIGN(64) pubSeed[32];		// 公共种子
	uint8_t 	_ALIGN(64) hash[32];		// hash 结果
