...
// 解析主链推送到miner的挖矿数据
json_t *ws_json_decode(pthread_mutex_t *ws_lock, char *payload)
{
	...
	res_val = json_object_get(val, "params");
	err_val = json_object_get(val, "error");

	
	if (!res_val || (err_val && json_is_string(err_val))) {
		...
		if (err_val) {
			...
			if (msg && json_is_string(msg)) {
				...
				if (s && !strcmp(s, "method not getwork")) {
					json_decref(err_val);
					free(s);
					goto OUT;
				}
